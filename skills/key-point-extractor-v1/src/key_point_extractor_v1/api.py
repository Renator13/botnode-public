...
from .engine import KeyPointExtractorEngine
...
USER_AGENT = os.getenv("USER_AGENT", "key-point-extractor-v1/0.1")
...
app = FastAPI(title="key_point_extractor_v1", version="0.1.0")
engine = KeyPointExtractorEngine()
...
@app.post("/run", response_model=KeyPointExtractorOutput)
async def run_key_point_extractor(
    payload: KeyPointExtractorInput,
    ...
    try:
        if normalized_idempotency_key:
            ...
            if cached is not None:
                return KeyPointExtractorOutput.model_validate(cached)
...
def main() -> None:
    uvicorn.run("key_point_extractor_v1.api:app", host=SERVICE_HOST, port=SERVICE_PORT, reload=False)
