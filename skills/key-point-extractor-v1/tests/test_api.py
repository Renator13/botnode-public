...
from key_point_extractor_v1 import api as api_module
from key_point_extractor_v1.models import KeyPointExtractorInput, KeyPointExtractorOutput
...
def test_idempotency_cache_reuses_previous_result(monkeypatch) -> None:
    ...
    async def fake_run(payload):  # type: ignore[no-untyped-def]
        calls["count"] += 1
        return KeyPointExtractorOutput(points=['A point'])

    monkeypatch.setattr(api_module.engine, "run", fake_run)
    payload = KeyPointExtractorInput(text='Long enough text for extraction.')

    first = asyncio.run(api_module.run_key_point_extractor(payload, idempotency_key="idem-1"))
    second = asyncio.run(api_module.run_key_point_extractor(payload, idempotency_key="idem-1"))
