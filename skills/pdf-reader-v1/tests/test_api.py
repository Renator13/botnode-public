...
from pdf_reader_v1 import api as api_module
...
def test_idempotency_cache_reuses_previous_result(monkeypatch) -> None:
    ...
    async def fake_run(payload):  # type: ignore[no-untyped-def]
        calls["count"] += 1
        return PdfReaderOutput(page_count=1, metadata={}, full_text='x')

    monkeypatch.setattr(api_module.engine, "run", fake_run)
    payload = PdfReaderInput(file_url='https://example.com/a.pdf')

    first = asyncio.run(api_module.run_pdf_reader(payload, idempotency_key="idem-1"))
    second = asyncio.run(api_module.run_pdf_reader(payload, idempotency_key="idem-1"))
