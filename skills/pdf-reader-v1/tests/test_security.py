...
from pdf_reader_v1.security import _request_with_retries, _validate_response_size, is_safe_public_http_url
...
def test_request_with_retries_uses_exponential_backoff(monkeypatch) -> None:
    ...
    monkeypatch.setattr("pdf_reader_v1.security.asyncio.sleep", fake_sleep)
    ...
    class FakeClient:
        ...
        async def request(self, **kwargs):  # type: ignore[no-untyped-def]
            ...
            return httpx.Response(200, request=httpx.Request("GET", "https://example.com"), content=b"%PDF")
...
def test_rejects_oversized_content_length_header() -> None:
    with pytest.raises(RuntimeError):
        _validate_response_size({"content-length": str((20 * 1024 * 1024) + 1)}, 20 * 1024 * 1024)
