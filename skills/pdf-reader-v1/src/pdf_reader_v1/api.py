...
from .engine import PdfReaderEngine
...
USER_AGENT = os.getenv("USER_AGENT", "pdf-reader-v1/0.1")
...
app = FastAPI(title="pdf_reader_v1", version="0.1.0")
engine = PdfReaderEngine(timeout_seconds=HTTP_TIMEOUT_SECONDS, user_agent=USER_AGENT, max_pdf_bytes=MAX_FILE_BYTES)
...
@app.post("/run", response_model=PdfReaderOutput)
async def run_pdf_reader(
    payload: PdfReaderInput,
    ...
    try:
        if normalized_idempotency_key:
            ...
            if cached is not None:
                return PdfReaderOutput.model_validate(cached)
...
def main() -> None:
    uvicorn.run("pdf_reader_v1.api:app", host=SERVICE_HOST, port=SERVICE_PORT, reload=False)
