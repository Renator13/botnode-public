...
from lead_enricher_v1 import api as api_module
from lead_enricher_v1.models import LeadEnricherInput, LeadEnricherOutput
...
def test_idempotency_cache_reuses_previous_result(monkeypatch) -> None:
    ...
    async def fake_run(payload):  # type: ignore[no-untyped-def]
        calls["count"] += 1
        return LeadEnricherOutput(company_name='Stripe', industry='Fintech', employees=5000, linkedin_url='https://linkedin.com/company/stripe', source='mock_db')

    monkeypatch.setattr(api_module.engine, "run", fake_run)
    payload = LeadEnricherInput(domain='stripe.com')

    first = asyncio.run(api_module.run_lead_enricher(payload, idempotency_key="idem-1"))
    second = asyncio.run(api_module.run_lead_enricher(payload, idempotency_key="idem-1"))
