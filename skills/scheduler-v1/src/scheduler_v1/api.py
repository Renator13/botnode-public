...
USER_AGENT = os.getenv("USER_AGENT", "scheduler-v1/0.1")
...
app = FastAPI(title="scheduler_v1", version="0.1.0")
...
@app.post("/run", response_model=SchedulerOutput)
    ...
    try:
        if normalized_idempotency_key:
            ...
            if cached is not None:
                return SchedulerOutput.model_validate(cached)
...
def main() -> None:
    uvicorn.run("scheduler_v1.api:app", host=SERVICE_HOST, port=SERVICE_PORT, reload=False)
