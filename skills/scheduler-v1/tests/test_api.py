...
from scheduler_v1 import api as api_module
from scheduler_v1.models import SchedulerInput, SchedulerOutput
...
def test_idempotency_cache_reuses_previous_result(monkeypatch) -> None:
    ...
    async def fake_run(payload):  # type: ignore[no-untyped-def]
        calls["count"] += 1
        return SchedulerOutput(job_id='job-1', status='scheduled', next_run='2026-01-01T00:00:00Z')

    monkeypatch.setattr(api_module.engine, "run", fake_run)
    payload = SchedulerInput(cron_expression='0 8 * * *', target_node_id='node-self', task_payload={'action':'run'})

    first = asyncio.run(api_module.run_scheduler(payload, idempotency_key="idem-1"))
    second = asyncio.run(api_module.run_scheduler(payload, idempotency_key="idem-1"))
