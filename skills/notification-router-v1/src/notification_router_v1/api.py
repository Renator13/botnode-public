...
from .engine import NotificationRouterEngine
...
USER_AGENT = os.getenv("USER_AGENT", "notification-router-v1/0.1")
...
app = FastAPI(title="notification_router_v1", version="0.1.0")
engine = NotificationRouterEngine(timeout_seconds=HTTP_TIMEOUT_SECONDS, user_agent=USER_AGENT)
    ...
    try:
        if normalized_idempotency_key:
            ...
            if cached is not None:
                return NotificationRouterOutput.model_validate(cached)
...
def main() -> None:
    uvicorn.run("notification_router_v1.api:app", host=SERVICE_HOST, port=SERVICE_PORT, reload=False)
