...
from notification_router_v1 import api as api_module
from notification_router_v1.models import NotificationRouterInput, NotificationRouterOutput
...
def test_idempotency_cache_reuses_previous_result(monkeypatch) -> None:
    ...
    async def fake_run(payload):  # type: ignore[no-untyped-def]
        calls["count"] += 1
        return NotificationRouterOutput(status='sent', provider_id='msg1', timestamp='2026-01-01T00:00:00Z')

    monkeypatch.setattr(api_module.engine, "run", fake_run)
    payload = NotificationRouterInput(channel='email', recipient='user@example.com', message_body='hello')

    first = asyncio.run(api_module.run_notification_router(payload, idempotency_key="idem-1"))
    second = asyncio.run(api_module.run_notification_router(payload, idempotency_key="idem-1"))
