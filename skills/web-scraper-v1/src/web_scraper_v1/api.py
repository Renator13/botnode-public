...
USER_AGENT = os.getenv("USER_AGENT", "web-scraper-v1/0.1")
...
app = FastAPI(title="web_scraper_v1", version="0.1.0")
engine = WebScraperEngine(timeout_seconds=HTTP_TIMEOUT_SECONDS, user_agent=USER_AGENT)
...
@app.post("/run", response_model=WebScraperOutput)
async def run_web_scraper(
    payload: WebScraperInput,
    ...
    try:
        if normalized_idempotency_key:
            ...
            if cached is not None:
                return WebScraperOutput.model_validate(cached)
...
def main() -> None:
    uvicorn.run("web_scraper_v1.api:app", host=SERVICE_HOST, port=SERVICE_PORT, reload=False)
