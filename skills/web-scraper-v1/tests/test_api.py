...
from web_scraper_v1 import api as api_module
from web_scraper_v1.models import WebScraperInput, WebScraperOutput
...
def test_idempotency_cache_reuses_previous_result(monkeypatch) -> None:
    ...
    async def fake_run(payload):  # type: ignore[no-untyped-def]
        calls["count"] += 1
        return WebScraperOutput(title='Example', text_content='Body', status_code=200, html_content=None)

    monkeypatch.setattr(api_module.engine, "run", fake_run)
    payload = WebScraperInput(url='https://example.com')

    first = asyncio.run(api_module.run_web_scraper(payload, idempotency_key="idem-1"))
    second = asyncio.run(api_module.run_web_scraper(payload, idempotency_key="idem-1"))
