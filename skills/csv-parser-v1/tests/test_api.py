...
from csv_parser_v1 import api as api_module
from csv_parser_v1.models import CsvParserInput, CsvParserOutput
...
def test_idempotency_cache_reuses_previous_result(monkeypatch) -> None:
    ...
    async def fake_run(payload):  # type: ignore[no-untyped-def]
        calls["count"] += 1
        return CsvParserOutput(total_rows=1, headers=['a'], data=[{'a':'1'}])

    monkeypatch.setattr(api_module.engine, "run", fake_run)
    payload = CsvParserInput(file_url='https://example.com/a.csv')

    first = asyncio.run(api_module.run_csv_parser(payload, idempotency_key="idem-1"))
    second = asyncio.run(api_module.run_csv_parser(payload, idempotency_key="idem-1"))
