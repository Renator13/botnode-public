...
from .engine import CsvParserEngine
...
USER_AGENT = os.getenv("USER_AGENT", "csv-parser-v1/0.1")
...
app = FastAPI(title="csv_parser_v1", version="0.1.0")
engine = CsvParserEngine(timeout_seconds=HTTP_TIMEOUT_SECONDS, user_agent=USER_AGENT, max_csv_bytes=MAX_FILE_BYTES)
...
@app.post("/run", response_model=CsvParserOutput)
async def run_csv_parser(
    payload: CsvParserInput,
    ...
    try:
        if normalized_idempotency_key:
            ...
            if cached is not None:
                return CsvParserOutput.model_validate(cached)
...
def main() -> None:
    uvicorn.run("csv_parser_v1.api:app", host=SERVICE_HOST, port=SERVICE_PORT, reload=False)
