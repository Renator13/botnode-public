...
from .engine import GoogleSearchEngine
from .models import GoogleSearchInput, GoogleSearchOutput


SERVICE_HOST = os.getenv("SERVICE_HOST", "0.0.0.0")
SERVICE_PORT = int(os.getenv("SERVICE_PORT", "8080"))
HTTP_TIMEOUT_SECONDS = float(os.getenv("HTTP_TIMEOUT_SECONDS", "15"))
USER_AGENT = os.getenv("USER_AGENT", "google-search-v1/0.1")
...
app = FastAPI(title="google_search_v1", version="0.1.0")
...
@app.post("/run", response_model=GoogleSearchOutput)
async def run_google_search(
    payload: GoogleSearchInput,
    ...
    try:
        if normalized_idempotency_key:
            ...
            if cached is not None:
                return GoogleSearchOutput.model_validate(cached)
...
def main() -> None:
    uvicorn.run("google_search_v1.api:app", host=SERVICE_HOST, port=SERVICE_PORT, reload=False)
