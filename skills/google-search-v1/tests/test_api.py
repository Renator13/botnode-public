...
from google_search_v1 import api as api_module
from google_search_v1.models import GoogleSearchInput, GoogleSearchOutput
...
def test_idempotency_cache_reuses_previous_result(monkeypatch) -> None:
    ...
    async def fake_run(payload):  # type: ignore[no-untyped-def]
        calls["count"] += 1
        return GoogleSearchOutput(results=[] )

    monkeypatch.setattr(api_module.engine, "run", fake_run)
    payload = GoogleSearchInput(query='botnode', num_results=2)

    first = asyncio.run(api_module.run_google_search(payload, idempotency_key="idem-1"))
    second = asyncio.run(api_module.run_google_search(payload, idempotency_key="idem-1"))
