...
from .models import BotNodeTask, CodeReviewOutput


logger = logging.getLogger("code_reviewer_v1.worker")
    ...
    async with httpx.AsyncClient(timeout=timeout) as client:
        while True:
            try:
                ...
                for task in tasks:
                    await _process_task(client, base_url, skill_id, api_key, task)
...
async def _process_task(
    ...
    output_data = await _run_local_service(client, task.input_data)
    ...
    complete_url = f"{base_url}/v1/tasks/complete"
    response = await _request_with_retries(
        client=client,
        method="POST",
        url=complete_url,
...
async def _run_local_service(client: httpx.AsyncClient, input_data: Dict[str, Any]) -> Dict[str, Any]:
    response = await _request_with_retries(
        client=client,
        method="POST",
    ...
    output = CodeReviewOutput.model_validate(response.json())
