...
from .engine import SentimentAnalyzerEngine
...
USER_AGENT = os.getenv("USER_AGENT", "sentiment-analyzer-v1/0.1")
...
app = FastAPI(title="sentiment_analyzer_v1", version="0.1.0")
...
@app.post("/run", response_model=SentimentAnalyzerOutput)
async def run_sentiment_analyzer(
    payload: SentimentAnalyzerInput,
    ...
    try:
        if normalized_idempotency_key:
            ...
            if cached is not None:
                return SentimentAnalyzerOutput.model_validate(cached)
...
def main() -> None:
    uvicorn.run("sentiment_analyzer_v1.api:app", host=SERVICE_HOST, port=SERVICE_PORT, reload=False)
