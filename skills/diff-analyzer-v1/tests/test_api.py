...
from diff_analyzer_v1 import api as api_module
from diff_analyzer_v1.models import DiffAnalyzerInput, DiffAnalyzerOutput
...
def test_idempotency_cache_reuses_previous_result(monkeypatch) -> None:
    ...
    async def fake_run(payload):  # type: ignore[no-untyped-def]
        calls["count"] += 1
        return DiffAnalyzerOutput(change_detected=True, change_score=1.0, summary='x', diff_snippet='y')

    monkeypatch.setattr(api_module.engine, "run", fake_run)
    payload = DiffAnalyzerInput(content_a='a', content_b='b')

    first = asyncio.run(api_module.run_diff_analyzer(payload, idempotency_key="idem-1"))
    second = asyncio.run(api_module.run_diff_analyzer(payload, idempotency_key="idem-1"))
