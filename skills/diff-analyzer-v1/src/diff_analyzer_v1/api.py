...
from .engine import DiffAnalyzerEngine
...
USER_AGENT = os.getenv("USER_AGENT", "diff-analyzer-v1/0.1")
...
app = FastAPI(title="diff_analyzer_v1", version="0.1.0")
engine = DiffAnalyzerEngine()
...
@app.post("/run", response_model=DiffAnalyzerOutput)
async def run_diff_analyzer(
    payload: DiffAnalyzerInput,
    ...
    try:
        if normalized_idempotency_key:
            ...
            if cached is not None:
                return DiffAnalyzerOutput.model_validate(cached)
...
def main() -> None:
    uvicorn.run("diff_analyzer_v1.api:app", host=SERVICE_HOST, port=SERVICE_PORT, reload=False)
